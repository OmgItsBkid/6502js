...
function SimulatorWidget(node) {
  ...
  function initialize() {
    ...
    $node.find('.binaryButton').click(assembler.binary);
    $node.find('.sourceButton').click(assembler.source);
  ...
  function UI() {
    ...
    function setState(state) {
      ...
      $node.find('.binaryButton').attr('disabled', !state.hexdump);
  ...
  function Memory() {
    ...
    function getRange(start, end){
      return memArray.slice(start,end);
    }
  ...
  function Assembler() {
    ...
    function binary(){
      //memory from 0x600 to 0x600 + codeLen
      var memSlice = memory.getRange(0x600, 0x600+codeLen);
      var typedArray = new Uint8Array(memSlice);
      var blob = new Blob([typedArray], { type: 'text/plain'});
      downloadBlob(blob, 'program.bin');
    }

    function source(){
      var code = $node.find('.code').val();
      var blob = new Blob([code], {type: 'text/plain'});
      downloadBlob(blob, 'program.asm');
    }
